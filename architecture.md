...
### Startup Cleanup
`app._cleanup_orphaned_uploads()` runs in the FastAPI lifespan on every startup. It deletes `wisper_upload_*` **and** `wisper_enroll_*` temp files left in `tempfile.gettempdir()` from requests that crashed mid-flight. The files are only needed for the duration of a single job/request, so anything on disk at boot time is safe to remove.
...
**R9-1 (`wisper_enroll_*`):** the standalone speaker-enroll route (`POST /speakers/enroll` in `routes/speakers.py`) is never handed off to a background job — its upload is only needed for the duration of that one request. The route's `try`/`finally` always deletes both the raw `wisper_enroll_*` upload and (if `convert_to_wav()` produced a separate file) the converted WAV, success or failure, so this startup sweep is purely the crash-window safety net for it, same role as it plays for `wisper_upload_*`.
