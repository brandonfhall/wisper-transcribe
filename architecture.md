...
├── aligner.py          Merge transcription segments with diarization labels; word-level splitting at turn boundaries when word timestamps are available, whole-segment max-overlap fallback otherwise (F8); micro-run smoothing pass (F13, `_smooth_word_speakers()`) absorbs a sandwiched run of ≤2 words or <1.0s between two same-speaker neighbors before grouping, so a jittered diarization boundary can't split one sentence across speaker blocks; R27: `_assign_word_speakers()` sorts diarization turns once and sweeps words against them (two-pointer "active" window) instead of an O(words × turns) brute-force scan per word — a 3-hour session's ~30k words × ~2k turns was ~60M overlap computations. The sweep's turn-expiry step is only valid when `words` is time-ordered (always true for whisper output); a single ascending-order check up front routes any out-of-order input to `_assign_word_speakers_bruteforce()` (the kept-verbatim original algorithm) instead of assuming the precondition, so output is unconditionally identical to the pre-R27 brute force — including tie-breaking, which the sweep reproduces by comparing original `diarization`-list indices among overlap ties rather than relying on sweep-visit order. `_best_overlap_speaker()` itself is unchanged and still used both by `align()`'s no-words fallback and by `_assign_word_speakers_bruteforce()`.
├── speaker_manager.py  Profile CRUD, embedding extraction, cosine-similarity matching, EMA updates; enroll_speaker_from_audio_dir() for per-user track enrollment (Phase 6) — prefers `*.wav` segments (SegmentedWavWriter output since R12), falling back to legacy `*.opus` as a best-effort attempt for pre-R12 recordings (those decode-fail gracefully rather than crashing, since they were never valid Opus streams); enroll_speaker() accepts an optional precomputed embedding= (skips its internal extract_embedding() call) so callers can average embeddings from multiple raw diarization labels before saving; _select_embedding_segments() (F10b) picks up to 5 segments preferring non-overlapping ("solo") 2-20s spans over the raw-longest segments, falling back when nothing fits that profile; R37: a module-level `_profiles_lock` guards every load-modify-save of the shared `speakers.json` store (remove_profile(), rename_profile(), reset_profiles(), and the JSON-store portion of enroll_speaker() — never the slow embedding-extraction/npy-write work, and never load_profiles()/save_profiles() themselves, which would deadlock the callers above that already hold the lock); remove_profile() is new in R37, consolidating what the CLI `speakers remove` command and the web `/speakers/{name}/remove` route used to each do unlocked and inline
...
├── campaign_manager.py Campaign CRUD: load/save/create/delete campaigns, add/remove roster members, bind_discord_id() / lookup_profile_by_discord_id() Discord ID binding, _validate_campaign_slug() / _validate_profile_key() delegate to path_utils; R37: a module-level `_campaigns_lock` guards every load-modify-save of the shared `campaigns.json` store (create/delete_campaign, add/remove_member, rekey_member, bind_discord_id, move/remove_transcript_from_campaign — never load_campaigns()/save_campaigns() themselves, which would deadlock the callers above that already hold the lock), mirroring recording_manager.py's per-record lock pattern for a single shared file instead of per-ID files
...
├── web/local_capture.py LocalCaptureManager: local mic + system-audio capture, mirroring BotManager's start()/stop()/start_session()/stop_session() shape but thread-based (not asyncio) and fully synchronous -- soundcard recorders are blocking pulls, so callers on the asyncio event loop must wrap the blocking calls in `asyncio.to_thread()`. Threading model: two capture threads (mic, system) only fill per-track byte FIFOs (`_ByteFifo`) via `resample_to_16k_mono()`; a single tick thread does ALL writing -- every ~20 ms (`ticker()`) it drains one 20 ms chunk from each FIFO (silence-padding a starved track, or draining the full surplus + padding the other track to match when a FIFO exceeds its ~2 s cap), writes each track's chunk to its own `SegmentedWavWriter`, and int32-sums the two into the combined writer (clipped to int16). The tick thread only ever exits its loop; `stop_session()` is the sole finaliser (join capture + tick threads, close writers, `concat_wav_segments()` the combined track, save) -- this prevents a double-finalise race between a naturally-exhausting ticker and an external stop. Injectable `capture_factory(device_id, samplerate) -> iterator[(np.ndarray, samplerate)]` (default `_soundcard_capture_factory`, lazy `soundcard` import + Windows `CoInitialize`/`CoUninitialize` per capture thread) and `ticker() -> iterator` (default real 20 ms wall-clock, tests inject a finite instant generator) mirror BotManager's `audio_source_factory` injection pattern. `enumerate_devices()` returns `{"microphones": [...], "loopbacks": [...], "available": bool}` -- `available: False` (never an exception) whenever `soundcard` is not importable or enumeration fails. `_active_recording` is deliberately never reset to `None` after a session finishes (matches BotManager) -- callers check `.status in {"recording", "degraded"}`, not `is not None`.
├── web/discord_bot.py  BotManager: start()/stop() lifecycle, start_session()/stop_session(), _session_loop with auto-rejoin (backoff [2,5,15,30,60]s), _route_frame → downsample_48k_stereo_to_16k_mono() + SegmentedWavWriter (per-user tracks; `__mixed__` — JDA's pre-mixed all-speakers track — written straight to the combined-track writer, bypassing auto-tag/unbound-speaker tracking) + auto-tag via lookup_profile_by_discord_id() + unbound_speakers tracking for real per-user IDs, _handle_disconnect (transient/permanent close code split), _finalise (closes writers, concat_wav_segments() merges the combined track into `combined.wav`, sets Recording.combined_path — R2); injectable audio_source_factory for testing; _unix_socket_source launches JDA sidecar subprocess (Java 25, JDAVE 0.1.8), _find_sidecar_jar() for JAR discovery, _read_frame() parses length-prefixed wire protocol over Unix socket
├── web/routes/record.py Full recording UI: JSON API /api/record/{start,stop} + /api/record/status (501 placeholder) + /api/record/channels (Discord REST proxy — lists guilds + voice channels via bot token); /api/record/devices (soundcard mic/loopback enumeration, always 200 with `available: false` when soundcard isn't importable), /api/record/start-local + /api/record/stop-local (local capture, mirroring start/stop; device ids are resolved to display names server-side via `enumerate_devices()`/`resolve_device_name()`, never a client-supplied name string) — both these and Discord's start routes call `_other_session_active()` for cross-manager mutual exclusion (409 if the *other* manager has a `"recording"`/`"degraded"` session; status-checked, not `is not None`, since neither manager ever clears `active_recording` back to `None`); /api/recordings (list, `?campaign=` filter), /api/recordings/{id} (detail), /api/recordings/{id}/transcribe, /api/recordings/{id}/delete (R7: these four back `wisper record list/show/transcribe/delete` — previously 501 stubs, now delegate to the same manager calls as the HTML routes below; `_recording_to_dict()` returns IDs/status/timestamps/derived booleans only, never a filesystem path — includes `source`/`devices` since F1 live-audio) + HTML routes GET /record (includes channel-browser panel + local device pickers, sourced from `enumerate_devices()`), POST /record/{start,stop}, GET /record/sse (SSE stream — `source` field added to the status payload), GET /recordings (campaign-grouped list), GET /recordings/{id}, POST /recordings/{id}/enroll, POST /recordings/{id}/transcribe, POST /recordings/{id}/delete, GET /recordings/{id}/live (501 placeholder); `_current_active_recording()` is the single resolver both the Record page and the SSE stream use to find whichever manager (Discord or local) currently has an active session, so a live local session no longer reports "idle"; the HTML and JSON transcribe routes share `_submit_recording_transcription()` (validation + JobQueue.submit + on_complete campaign hand-off) so both entry points do the identical thing; _validate_recording_id() + _uid_guard path-traversal guards; Discord preset quick-select dropdown; pre-fills default guild/channel from config
├── models.py           Dataclasses: Word, TranscriptionSegment (+ words: Optional[list[Word]]), DiarizationSegment, AlignedSegment, SpeakerProfile, CampaignMember (+ discord_user_id), Campaign, Recording (+ unbound_speakers, source: str = "discord", devices: dict — local-capture additions, default keeps legacy JSON loading unchanged), SegmentRecord, RejoinAttempt, Edit, SpeakerSuggestion, LootChange, NPCMention, SummaryNote
...
└── web/                FastAPI web UI + Discord recording bot infrastructure
    ├── app.py          FastAPI application factory (create_app()), module-level app instance for uvicorn; lifespan wires up both BotManager (Discord) and LocalCaptureManager (local capture) onto app.state, stopping LocalCaptureManager via `asyncio.to_thread()` on shutdown since its stop() joins real threads
    ...
    ├── audio_writer.py SegmentedWavWriter (rotating 60-s self-contained 16 kHz mono 16-bit WAV segments via stdlib `wave`, sample-count rotation; `writeframes()` patches the RIFF/data chunk sizes on every call so a crash mid-segment still leaves a readable file), downsample_48k_stereo_to_16k_mono() (pure NumPy: average L+R, 3-tap moving-average low-pass, decimate by 3 — no audioop, removed in 3.13; fixed 48k Discord wire format), resample_to_16k_mono() (local-capture counterpart: arbitrary-samplerate float32 blocks from `soundcard`, via `scipy.signal.resample_poly` at a `gcd`-reduced rational ratio — handles the non-integer 44.1k->16k case downsample_48k_stereo_to_16k_mono can't; scales [-1.0, 1.0] float samples to int16 range before clip/round), concat_wav_segments() (R2: merges a writer's rotated segments into one WAV, skipping unreadable/empty segments, returns None if nothing was written)
    ...
    └── routes/
        ├── __init__.py     Jinja2 templates setup, shared get_queue() helper, get_bot_manager()/get_local_capture_manager() app.state accessors, urlencode filter
...
**R36 — tqdm monkey-patching is load-bearing in three layers.** `tqdm.write`/`tqdm.__init__` are process-global mutable state, and three independent, unrelated call sites patch them for three different purposes:
1. **`debug_log.Logger._patch_tqdm`** (above) — a *permanent* tee, installed once at CLI/server startup, that mirrors every `tqdm.write()` call into the `--debug` log file for the lifetime of the process. Idempotent as of R32-10 (re-patching always wraps the true original, never a previous tee, so repeated `setup_logging()` calls in one process don't stack wrapper closures).
2. **`jobs._run_transcription_job`** (web) — a *per-job* capture, installed when a job starts and restored when it ends, that redirects `tqdm.write()` into `job.log_lines` for the SSE progress stream (see "Job Queue" below) and is also where `JobQueue.cancel()`'s `threading.Event` is checked, so **cancellation only fires when tqdm writes** — a job stuck in a long non-tqdm-emitting call (e.g. an LLM request) cannot be interrupted until the next tqdm write.
3. **`pipeline._patch_tqdm_for_queue`** (`parallel_stages=True`) — a *per-subprocess* patch, installed inside each `ProcessPoolExecutor` worker before any ML import, that redirects `tqdm.write`/`tqdm.__init__` into the IPC queue described under "Parallel stage processing" below instead of the worker's own stderr.

All three coexist safely only because of the **one-job-at-a-time invariant** (`JobQueue` runs a single job at a time; see Non-Obvious Gotchas in CLAUDE.md): layer 2 patches and un-patches around each job's execution, so there is never a second job's patch to collide with, and layer 1's permanent tee is always still reachable underneath (or on top of) layer 2's temporary one, since neither layer stacks indefinitely (R32-10) — only chains one level deep. Layer 3 is scoped to a subprocess's own `tqdm` module instance and never touches the parent process's patched function at all. None of these layers know about each other; this note is the only place their interaction is described end to end.
...
└── recordings/
    ...
    └── <recording_id>/
        ...
        ├── per-user/
        │   └── <discord_user_id>/
        │       ├── 0000.wav         60-s self-contained 16 kHz mono 16-bit WAV segment (v1 file-format invariant 4)
        │       └── 0001.wav
        ├── combined/
        │   ├── 0000.wav             60-s self-contained WAV segment of JDA's pre-mixed `__mixed__` track
        │   └── 0001.wav
        └── combined.wav             all combined/ segments concatenated by _finalise() (R2), copied to output/ before JobQueue submit
```
...
- **`source`/`devices` (local capture)** — `Recording.source` is `"discord"` (default) or `"local"`; `create_recording()` takes optional `source=`/`devices=` params, and `_recording_from_dict()` defaults both to `"discord"`/`{}` via `d.get(...)` so pre-existing `metadata.json` files without these keys still load. `devices` holds display-only device names (e.g. `{"mic": "Built-in Microphone", "system": "Speakers (loopback)"}`) for the recording detail page — never a filesystem path, never used to build one. Local sessions pass `voice_channel_id=""`/`guild_id=""`; `reconcile_on_startup()` is source-agnostic and needs no change.
- **`web/local_capture.py`'s `LocalCaptureManager`** — the local (mic + system audio) counterpart to `BotManager`, writing into the identical `recordings/<id>/per-user/<track>/NNNN.wav` + `recordings/<id>/combined/NNNN.wav` → `combined.wav` layout (invariants 1-5 above hold unchanged; `<track>` is `"mic"`/`"system"` in place of a Discord user ID). See the Module Map entry above for the threading model.

`web/audio_writer.py` provides:
- **`SegmentedWavWriter`** — writes 16 kHz mono 16-bit PCM into rotating self-contained WAV files via the stdlib `wave` module. Rotation is triggered by sample count (media time) rather than wall-clock time, so tests that feed frames faster than real-time work correctly. On construction, it scans the target directory for existing `*.wav` files and starts at the next index, enabling crash recovery by a new writer instance. Crash safety comes from `wave.Wave_write.writeframes()` itself: it patches the RIFF/`data` chunk sizes in the header on every call after the first, and `write()` additionally flushes the file after each call, so a killed process leaves the current segment's header self-consistent with whatever was actually written (only rotated/closed segments are guaranteed fully durable via `fsync`).
- **`downsample_48k_stereo_to_16k_mono()`** — converts JDA's native 48 kHz stereo 16-bit PCM to 16 kHz mono 16-bit in pure NumPy (average L+R, 3-tap moving-average low-pass, decimate by 3); Python 3.13 removed `audioop`, and this is more than adequate for speech destined for Whisper/pyannote (both resample to 16 kHz internally anyway). Applied at write time to both per-user tracks and the combined track — storing 16 kHz mono instead of 48 kHz stereo is ~6x less disk.
- **`resample_to_16k_mono()`** — the local-capture counterpart used by `web/local_capture.py`. `soundcard` delivers float32 PCM at the device's native rate (typically 44100 or 48000 Hz, not guaranteed to be an integer ratio of 16000 the way Discord's fixed 48 kHz is), so this uses `scipy.signal.resample_poly` at a `math.gcd`-reduced rational ratio (`up/down`) instead of the fixed decimate-by-3 the Discord path uses. Averages multi-channel blocks to mono first, then scales the [-1.0, 1.0] float range to int16 before clip/round — omitting that scale factor rounds everything to silence.
- **`concat_wav_segments()`** — merges a writer's rotated segments (same params throughout, so plain frame concatenation suffices) into one output WAV; skips unreadable/empty segments with a warning rather than aborting the whole merge, and returns `None` if nothing was written.

This replaces the pre-R12 `SegmentedOggWriter`/`RealtimePCMMixer` pair: the old writer wrote raw 48 kHz stereo PCM frames into hand-rolled Ogg pages as though they were pre-encoded Opus packets (producing files no Opus decoder could read), and the mixer was driven once per *incoming* frame rather than once per real 20 ms tick, so N concurrent speakers advanced the combined track N × 20 ms per real 20 ms (senior-review finding R12).

**Five v1 file-format invariants (versioned contract for future live transcription in v2):**
1. Each segment file is a self-contained WAV file whose header's RIFF/`data` chunk sizes match the audio actually written to it.
2. Segment manifest is append-only and atomic (per-recording mutex + atomic file replace).
3. Segment length ≤ 60 s (3000 chunks × 20 ms at 16 kHz).
4. Per-user directory layout `recordings/<id>/per-user/<discord_id>/NNNN.wav` is fixed.
...
**Combined-track finalisation (R2):** JDA sends a `__mixed__` frame per real 20 ms tick (its own pre-mixed all-speakers track — see `handleCombinedAudio` in `discord-bot`'s `Main.java`) alongside per-user frames; `_route_frame` writes it straight to the combined-track `SegmentedWavWriter`, so the combined track's duration always tracks wall-clock time regardless of how many users are speaking. `BotManager._finalise()` closes all writers, then calls `concat_wav_segments()` on the combined-track segment directory and assigns the result to `Recording.combined_path` when there was any audio; an empty/no-frames session leaves `combined_path` as `None`, which `recording_transcribe_html` still reports as `?error=no_audio`. Before this fix, `combined_path` was assigned exactly once at recording creation (to `None`) and never updated, so the transcribe hand-off always failed.
...
- `tests/test_audio_writer.py` (R12) covers `SegmentedWavWriter` rotation at 60 s, three-segment sessions, valid-WAV params (rate/channels/sampwidth) verification, crash-recovery (second writer resumes from next index; the "crashed" segment is still readable because `writeframes()` patches the header on every call), write return values, empty-write no-op; `downsample_48k_stereo_to_16k_mono()` (correct 960→320 sample count, empty input, silence, L/R averaging, a ramp survives low-pass+decimation as still-monotonic and non-silent, int16 clipping); `concat_wav_segments()` (no segments → None, multi-segment merge duration equals the sum of segment durations, an unreadable/zero-byte segment is skipped rather than aborting the merge, all-unreadable → None) (18 tests)
- `tests/test_record_routes.py` covers start (201 + recording_id), missing voice_channel_id (400), stop with no session (400), path-traversal rejection, server.json lifecycle (written on startup, deleted on shutdown), GET /record returns 200, GET /recordings empty state + campaign-grouped list, GET /recordings/{id} detail + unknown-id 303 redirect, POST /recordings/{id}/delete removes entry, GET /recordings/{id}/live returns 501, POST /recordings/{id}/enroll (valid → 303 + profile created, invalid discord_user_id → 400, user not in unbound_speakers → 409), R2 end-to-end regression driving a real `BotManager` session (fake audio source) through `/recordings/{id}/transcribe` and confirming `combined_path` is populated and the hand-off no longer redirects with `?error=no_audio`; R7: the JSON API (`GET /api/recordings` empty + populated + `?campaign=` filter + no leaked filesystem paths, `GET /api/recordings/{id}` detail + 404, `POST /api/recordings/{id}/transcribe` 202 + job id echoed + 409 not-ready + 422 no-audio + 404 unknown id, `POST /api/recordings/{id}/delete` 200 + 404 unknown id); live local recording Phase 1: `GET /api/record/devices` (unavailable via `sys.modules["soundcard"] = None`, enumerated result passthrough when available), `POST /api/record/start-local`/`stop-local` (201 with resolved device names in `devices`, 400 missing ids, 503 when unavailable or manager not wired, 400 stop with nothing active) — using a `LocalCaptureManager` swapped onto `app.state.local_capture_manager` with a scripted (no real audio, no soundcard import) capture_factory + instant ticker; cross-manager mutual exclusion in both directions (a fake "already active" `BotManager` stand-in is used for the Discord-active-blocks-local direction, since the real `BotManager.start_session()`'s async session loop races to mark the recording `"failed"` almost immediately with no Discord token configured in tests); `_current_active_recording()` exercised directly (not over the live, infinitely-polling `/record/sse` HTTP stream, which has no clean "read just the first event" pattern under TestClient's synchronous wrapper) confirming a local session is reported instead of "idle"
- `tests/test_record_cli.py` covers "server not running" error, server.json discovery → HTTP POST, WISPER_SERVER_URL env var override, list output, recording_id validation, stop → server POST, transcribe with path-traversal guard, delete with path-traversal guard, start missing --voice-channel error, show/transcribe valid IDs request server, token masking in config show, config discord wizard prompts, empty-input preserves existing token; R7: an end-to-end test that drives `record list/show/transcribe/delete` through the real `create_app()` FastAPI app (not a mocked `httpx.request`) to prove the CLI and the live JSON API actually agree, since a mocked-httpx test can't catch a stub-vs-real-route mismatch
- `tests/test_discord_bot.py` covers BotManager start/stop lifecycle, start_session recording persisted, per-user .wav files written from PCM frames (correct rate/channels/sampwidth), transient 4015 rejoin logged, exhausted retries → degraded, permanent 4014 → failed (no retry), stop_session → completed, known Discord ID auto-tagged to profile key on first frame, unknown Discord ID gets empty string, unknown Discord ID added to unbound_speakers, known Discord ID NOT added to unbound_speakers, 3-user simultaneous interleaved frames → all per-user dirs populated, 3 unknown speakers all in unbound list (no duplicates), simultaneous known+unknown speakers (tagged vs unbound split); R12: `__mixed__` frames never create per-user state (no dir, no discord_speakers/unbound_speakers entry), combined-track duration tracks wall-clock ticks rather than concurrent-speaker count (the N×20ms regression test); R2: `_finalise()` concatenates rotated combined-track segments into `combined.wav` with `combined_path` set and merged duration equal to the sum of the segments', and a no-frames session leaves `combined_path` as `None`; all via injected fake audio sources (no real JDA/Discord) (18 tests)
...
- `tests/test_local_capture.py` covers `LocalCaptureManager` via a scripted `capture_factory` (blocks keyed by device id; a device with no entry never populates its FIFO) + a finite instant `ticker` (no real-time delay) — no real soundcard/audio devices or Win32 calls anywhere: start_session (local `Recording` created with empty `voice_channel_id`/`guild_id`, `devices` populated, already-active raises `RuntimeError`, device names fall back to the raw id), per-track WAV output (correct rate/channels/sampwidth, duration == tick count × 20 ms), combined-track duration, silence substitution for a track that never provides frames, stop_session (`"completed"` status + `combined_path` set, persisted to disk), a no-frames session leaving `combined_path` as `None`, `stop_session()`/`stop()` no-ops when nothing is active; direct (thread-free) unit tests of `_do_tick()` covering the mix-two-tracks arithmetic, starved-track silence padding, and FIFO-overflow surplus draining + other-track padding; `_ByteFifo` push/drain; `enumerate_devices()` (unavailable when `soundcard` isn't importable via `sys.modules` injection, splits mics vs. loopbacks via `isloopback`, degrades to unavailable rather than raising on an enumeration exception) and `resolve_device_name()`. Duration-sensitive tests join the manager's capture + tick threads to their natural (finite-ticker) completion before calling `stop_session()`, since `stop_session()` also sets the production-facing `stop_event` the tick loop checks every iteration — calling it immediately after `start_session()` would otherwise race and could truncate the session before all scripted ticks run (tests that only assert status/existence don't need this).
...
- Progress: `tqdm.write` is monkey-patched per-job to capture log lines into `job.log_lines`; `tqdm.__init__` is also patched to redirect the progress bar to `job.progress`; both are restored after completion. In parallel mode, `capturing_write` also detects `[progress:channel]` prefixed messages forwarded by the drain thread and routes them to `job.progress_channels[channel]` (a `dict[str, str]` keyed by `"transcribe"` / `"diarize"`) rather than `log_lines`. See "R36 — tqdm monkey-patching is load-bearing in three layers" under Logging above for how this per-job patch, `debug_log`'s permanent tee, and `pipeline`'s per-subprocess patch coexist.
