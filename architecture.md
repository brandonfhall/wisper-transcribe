...
└── web/                FastAPI web UI + Discord recording bot infrastructure
    ...
    ├── enroll_shared.py Shared enrollment-wizard logic used by both routes/transcribe.py and routes/transcripts.py: build_legacy_label_map() (raw label → current display name), template_current_names() (filters raw-label-valued entries for form prefill), apply_enrollment_submit() (rename + enroll-or-EMA-update + campaign add, single implementation for both entry points)
...
├── <stem>_diar.json                enrollment sidecar: diarization_segments, input_path (now the durable <stem><suffix> path above), campaign slug
...
3. `_extract_speaker_excerpts()` parses the output markdown for each speaker's first timestamp and cuts a ~12 s clip via ffmpeg, saved to `{stem}_excerpt_{speaker}.mp3` alongside the transcript. The first dialogue line per speaker is saved to `{stem}_excerpt_{speaker}.txt`. Both survive server restarts.
4. `_write_enrollment_sidecar()` writes `{stem}_diar.json` alongside the transcript containing the raw `diarization_segments`, the (now durable, per step 2) `input_path`, and `campaign` slug. This is the key artifact that makes the transcript-centric wizard restart-safe.
5. `pipeline.process_file()` writes `job_id` to the transcript's YAML frontmatter for legacy linking.

**Shared enrollment logic (`web/enroll_shared.py`):**
- `build_legacy_label_map(md_path, segments)` maps raw pyannote label → *current* display name in the transcript body, by interval-matching markdown timestamps against diarization segment `[start, end]` spans (falls back to nearest-start-time when no interval contains the timestamp — whisper segment starts routinely fall just outside pyannote turns). Both routes call this before renaming — without it, once *any* profile is enrolled `match_speakers` writes display names into the transcript, so a rename keyed on the raw `SPEAKER_XX` string becomes a silent no-op on every session after the first (F1).
...
- `apply_enrollment_submit(md_path, segments, input_path, campaign_slug, device, renames)` is the single submit implementation both routes call:
  1. Resolves `current_names` via `build_legacy_label_map`, then drops any submission whose *new* name is raw-label-shaped (`^SPEAKER_\d+$`) — refusing to ever rename/enroll a raw-label-named "speaker" (F2 server-side enforcement).
  2. Renames in a single pass against content read once (`formatter.update_speaker_names`), skipping any entry where the resolved old name already equals the new name.
  3. Groups remaining renames by *target display name* (handles two raw labels being assigned the same name in one submit — common pyannote over-segmentation) and, per group, decides: skip entirely if unchanged and a profile already exists under that key; if the profile exists, extract embeddings for each raw label in the group (averaging when there's more than one) and merge via `speaker_manager.update_embedding()` (EMA, alpha=0.3) — never `enroll_speaker()`, which would overwrite the embedding and clobber `display_name`/`enrolled_date`/etc; if the profile is new, call `enroll_speaker()`, passing a pre-averaged `embedding=` when the group has more than one raw label (F3).
  4. Adds the profile to the job/transcript's campaign (if any) after each successful enroll/update, same `add_member`-if-not-already-a-member guard as before.
  `speaker_manager.enroll_speaker()` grew an optional `embedding: Optional[np.ndarray]` parameter for step 3 — when provided it's saved as-is instead of calling `extract_embedding()` internally, which is what lets the caller average across multiple raw labels before persisting.
...
7. `GET /transcripts/{name}/enroll` reads speaker labels from the sidecar (always the original `SPEAKER_XX` labels regardless of whether the transcript has already been renamed), locates clip files on disk, and renders the wizard. This works after any server restart. The route builds `current_names` via `build_legacy_label_map()` (filtered through `template_current_names()`) and passes it to the template so the input fields pre-fill with names the user previously applied — letting them re-enter the wizard to correct a typo without retyping every name, while an untouched field stays empty. **F5:** the route also checks whether the sidecar's `input_path` exists on disk and passes `audio_missing` to the template; `speaker_enroll.html` renders a warning banner ("Voice enrollment unavailable — source audio missing. Name changes will still apply.") above the form when true, so the user knows *before* submitting rather than being surprised after. The job-centric `GET /transcribe/jobs/{id}/enroll` does the same check against `job.input_path`.
8. `GET /transcripts/{name}/excerpt/{speaker_name}` serves the audio clip from disk.
9. `POST /transcripts/{name}/enroll` reconstructs `DiarizationSegment` objects from the sidecar and calls `enroll_shared.apply_enrollment_submit()`, which now returns an `EnrollmentResult(current_names, audio_missing)` (F5) instead of a bare dict. If the source audio no longer exists at `input_path`, renaming still completes but embedding extraction is skipped with a `log.warning`, `audio_missing` comes back `True`, and the route appends a generic `?notice=enroll_audio_missing` query flag to the redirect (never the path or exception text, per the security rules below). `transcript_detail.html` renders a matching banner ("Speaker names were updated, but voice enrollment was skipped because the source audio is no longer available.") when that flag is present. The job-centric `POST /transcribe/jobs/{id}/enroll` does the same, appending the flag to its `/transcripts/{name}` redirect.

**Legacy job-based wizard (transcribe.py) — available while server session is live:**
- `GET /transcribe/jobs/{id}/enroll` renders the same wizard using in-memory `job.diarization_segments` (prefers these over frontmatter, so re-opening after a rename still shows original `SPEAKER_XX` labels). Falls back to frontmatter if segments are absent. Also builds `current_names` via `build_legacy_label_map()`/`template_current_names()` from `job.diarization_segments` and `job.output_path` — this path previously never resolved current names at all, which was the direct cause of F1 (every rename after the first session silently no-op'd).
...
- `POST /transcribe/jobs/{id}/enroll` calls the same `enroll_shared.apply_enrollment_submit()` as the transcript-centric path, passing `job.diarization_segments`, `job.input_path`, and `job.kwargs.get("campaign")`.
