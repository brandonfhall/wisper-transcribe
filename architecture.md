...
- **`JOB_ENROLL` (Phase 2.5):** the speaker-enrollment wizard's slow half. `submit_enroll(md_path, transcript_name, groups, device)` carries only the transcript path, the validated rename groups (`display_name -> [raw_label, ...]`, from `enroll_shared.apply_renames()`), and the device — deliberately not the diarization segments or source audio path, to keep the job payload small. `output_path` is set to `md_path` immediately at submit time (not just on completion, unlike the LLM jobs) so "View transcript" works while the job is still running — the rename already happened synchronously in the route; only embedding extraction is pending. `_run_enroll_job()` re-reads the transcript's `<stem>_diar.json` sidecar for `segments`/`input_path`/`campaign` (restart-irrelevant since the queue itself is in-memory anyway) and calls `enroll_profiles()` with a `progress` callback that appends straight into `job.log_lines` (no tqdm/stderr capture needed, unlike transcription/LLM jobs). On missing source audio (checked both before enqueue in the route and again in the runner, since the file could vanish between the two), or on any other exception from `enroll_profiles()` (e.g. a WAV-conversion failure), the job fails with a **generic** error string (`"Source audio not available"` / `"Enrollment failed"`) — never `str(exc)`, since `job.error` renders directly into the job detail page's HTML and a WAV-conversion exception message can contain a filesystem path. Unlike `_run_llm_job`, the exception is **not** re-raised after being caught, because `_worker()`'s own except-block would otherwise overwrite the generic message with `str(exc)`.
...
- **Phase 2.5 split** — what used to be one synchronous function (`apply_enrollment_submit`: rename + WAV convert + embedding extraction, all inline in the HTTP request) is now two, so the slow half can run in a `JOB_ENROLL` job instead of blocking the browser tab for 30–120s:
  1. **`apply_renames(md_path, segments, renames)`** — fast, synchronous, stays inline in the route. Resolves `current_names` via `build_legacy_label_map`, drops any submission whose *new* name is raw-label-shaped (`^SPEAKER_\d+$`) (F2), renames in a single pass against content read once (`formatter.update_speaker_names`) skipping entries where the resolved old name already equals the new name, then groups remaining *eligible* renames by *target display name* into a `RenameResult(current_names, groups)` — `groups` maps display name → `[raw_label, ...]`, handling two raw labels assigned the same name in one submit (common pyannote over-segmentation). A raw label is excluded from `groups` when unchanged and a profile already exists under that key (F3's "nothing to do" skip). `groups` is empty when nothing submitted was eligible.
