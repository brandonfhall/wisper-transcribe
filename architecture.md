...
- **Local-provider model autocomplete**: `GET /config/ollama-status` and `GET /config/lmstudio-status` query `/api/tags` / `/v1/models` respectively (3 s connect timeout, endpoint read from saved config — no user input reaches `httpx`) and return `{"running": bool, "models": [{"name", "size"}]}`. The config page renders a single `llm_model` text input bound to a `<datalist id="llm_model_options">`; JS populates the datalist from the active provider's status endpoint so the browser shows installed/loaded models as autocomplete suggestions while still allowing free-text entry. A status line under the input ("✓ Ollama running · N models available") and a ↻ Refresh button live in `#llm_local_status_row`, shown only for local providers. A shared `_LOCAL_PROVIDERS` JS map drives both Ollama and LM Studio with one `loadLocalProviderStatus()` function; the datalist is cleared on switch to a cloud provider so stale suggestions never appear.
...
- **Local-provider datalist XSS prevention**: the JS that populates `<datalist id="llm_model_options">` uses `document.createElement('option')` with `.value` / `.label` property assignment (never `innerHTML` or string-built HTML) so model names returned by Ollama or LM Studio cannot inject markup.
