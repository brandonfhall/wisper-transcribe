...
├── web/routes/record.py Full recording UI: JSON API /api/record/{start,stop} + /api/record/status (501 placeholder) + /api/record/channels (Discord REST proxy — lists guilds + voice channels via bot token); /api/recordings (list, `?campaign=` filter), /api/recordings/{id} (detail), /api/recordings/{id}/transcribe, /api/recordings/{id}/delete (R7: these four back `wisper record list/show/transcribe/delete` — previously 501 stubs, now delegate to the same manager calls as the HTML routes below; `_recording_to_dict()` returns IDs/status/timestamps/derived booleans only, never a filesystem path) + HTML routes GET /record (includes channel-browser panel), POST /record/{start,stop}, GET /record/sse (SSE stream), GET /recordings (campaign-grouped list), GET /recordings/{id}, POST /recordings/{id}/enroll, POST /recordings/{id}/transcribe, POST /recordings/{id}/delete, GET /recordings/{id}/live (501 placeholder); the HTML and JSON transcribe routes share `_submit_recording_transcription()` (validation + JobQueue.submit + on_complete campaign hand-off) so both entry points do the identical thing; _validate_recording_id() + _uid_guard path-traversal guards; Discord preset quick-select dropdown; pre-fills default guild/channel from config
