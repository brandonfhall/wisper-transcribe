...
├── web/discord_bot.py  BotManager: start()/stop() lifecycle, start_session()/stop_session(), _session_loop with auto-rejoin (backoff [2,5,15,30,60]s), _route_frame → SegmentedOggWriter + RealtimePCMMixer + auto-tag via lookup_profile_by_discord_id() + unbound_speakers tracking, _handle_disconnect (transient/permanent close code split), _finalise; injectable audio_source_factory for testing; _unix_socket_source placeholder (Phase 9)
├── web/routes/record.py Full recording UI: JSON API /api/record/{start,stop,status} + HTML routes GET /record, POST /record/{start,stop}, GET /record/sse (SSE stream), GET/DELETE /recordings, GET /recordings/{id}, POST /recordings/{id}/enroll (unknown-speaker enrollment), GET /recordings/{id}/live (501 placeholder); _validate_recording_id() + _uid_guard path-traversal guards; campaign-grouped recordings list (Pattern 7)
