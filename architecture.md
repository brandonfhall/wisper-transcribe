...
├── journal.py          Rolling campaign journal: folds session .summary.md sidecars one-at-a-time into campaigns/<slug>/journal.md (bounded-context LLM rewrite); tracks folded sessions in journaled_sessions: frontmatter
...
└── web/                FastAPI web UI + Discord recording bot infrastructure
    ...
    ├── jobs.py         In-memory job queue, JobQueue class, asyncio background worker, SSE progress via tqdm.write patch; LLM job types (refine/summarize/campaign_journal) with stderr capture; submit_journal()/_run_journal_job() fold session summaries into the rolling campaign journal; on completion, moves a wisper_upload_* temp file next to the transcript as <stem><suffix> (durable across restarts, deletes it instead when there's no diarization data or the job fails/is cancelled — F5); _write_enrollment_sidecar() persists diarization segments + speaker_map (F7) + the (now durable) audio path to <stem>_diar.json alongside the transcript for restart-safe enrollment
    ├── audio_writer.py SegmentedWavWriter (rotating 60-s self-contained 16 kHz mono 16-bit WAV segments via stdlib `wave`, sample-count rotation; `writeframes()` patches the RIFF/data chunk sizes on every call so a crash mid-segment still leaves a readable file), downsample_48k_stereo_to_16k_mono() (pure NumPy: average L+R, 3-tap moving-average low-pass, decimate by 3 — no audioop, removed in 3.13), concat_wav_segments() (R2: merges a writer's rotated segments into one WAV, skipping unreadable/empty segments, returns None if nothing was written)
    ...
    └── routes/
        ...
        ├── campaigns.py    GET/POST /campaigns, campaign detail, roster add/remove, delete; POST /campaigns/{slug}/transcripts/remove (unlink transcript from campaign without deleting the file); POST /campaigns/{slug}/journal (queue a JOB_CAMPAIGN_JOURNAL fold → redirect to job page) + GET /campaigns/{slug}/journal (render the rolling journal)
...
- **`journal.py` — campaign-scoped, incremental.** Where `summarize.py` is per-session, `journal.py` accumulates sessions into one living `campaigns/<slug>/journal.md`. `update_journal()` folds **one** session at a time: the LLM receives `[current journal body] + [one new .summary.md]` via the free-text `complete()` method and returns a rewritten journal. This keeps context **bounded** — prompt size is independent of campaign length, so a 50-session campaign costs the same per fold as session 2. Folded sessions are recorded in the journal's `journaled_sessions:` YAML frontmatter (a list, so out-of-order summarization still diffs correctly); `unjournalled_sessions()` returns campaign transcripts that have a `.summary.md` but are not yet in that list. The session `.summary.md` sidecars are the source of truth and are never modified. Design rationale and the three sibling features (combined summary, "previously on…" recap, hierarchical summaries) live in `plan.md`.
