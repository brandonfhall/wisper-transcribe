...
- **Startup cleanup** — `app._cleanup_orphaned_uploads()` runs on every startup and deletes `wisper_upload_*` temp files. Since F5 this is only the crash-window safety net: `JobQueue.submit()` renames the temp file to a friendly name immediately (so running jobs never match the glob), and completed jobs either move the audio next to the transcript (durable, backs the enrollment wizard) or delete it. Never point anything long-lived at a `wisper_upload_*` path.
...
- **One job at a time.** `_model`, `_pipeline`, and `_embedding_model` are module-level globals — not thread-safe. `JobQueue` runs one job at a time intentionally; this covers transcription, LLM (refine/summarize), and enroll (`JOB_ENROLL`) jobs alike.
