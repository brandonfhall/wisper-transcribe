...
### Phase 1 implementation notes (pinned 2026-08-15, after reading the code — build from these)

- **New module `web/local_capture.py`**, mirroring `BotManager`'s shape but *simpler*: no reconnect/backoff/close-code machinery, no token resolution. Thread-based, not asyncio — soundcard recorders are blocking pulls (`recorder.record(numframes)` blocks until ~20 ms of audio exists). `LocalCaptureManager(data_dir, capture_factory=None, ticker=None)` with `start_session(campaign_slug, mic_device_id, system_device_id) → Recording`, `stop_session()`, `active_recording` property, and `start()/stop()` lifecycle methods so `app.py` lifespan wiring matches BotManager/JobQueue.
- **Injection seams for tests** (the `audio_source_factory` trick, adapted): `capture_factory(device_id, samplerate) → iterator of (numpy float32 array shape (n, ch), samplerate)` blocks — default implementation wraps soundcard; tests inject scripted generators and never import soundcard. `ticker() → iterator` yielding once per 20 ms wall-clock — tests inject a finite instant generator, keeping the R12-style "media time not wall time" testability. `soundcard` itself is imported *lazily inside functions* and mocked via `sys.modules` injection where needed (same trick as `tests/test_llm_clients.py`).
- **Threading/data-flow model — capture threads only fill FIFOs; the tick thread does ALL writing.** Two capture threads (mic, system) resample each block to 16 kHz mono int16 and append to per-track byte FIFOs (`collections.deque` + lock, or a `bytearray` under a lock). The tick thread, each 20 ms tick, drains up to 320 samples (20 ms @ 16 kHz) from each FIFO — **substituting silence for a starved track** — writes each track's 320-sample chunk to that track's `SegmentedWavWriter` (`recordings/<id>/per-user/mic/`, `.../system/`), sums the two (int32 accumulate, clip to int16) and writes to the combined writer. Rationale: (a) WASAPI loopback delivers **no frames while the system plays nothing** — silence substitution keeps all three tracks wall-clock continuous and mutually aligned, which Phase 2's timestamp-based energy attribution depends on; (b) one writer thread means no new concurrency surface. Bound each FIFO (~2 s); if a FIFO exceeds the cap (device clock faster than tick clock), drain the surplus into the same tick's write rather than dropping audio.
- **Windows COM init**: each capture thread calls `CoInitialize` before touching soundcard (`import comtypes`/`ctypes.windll.ole32.CoInitialize(None)` — soundcard's documented threading caveat) and uninitializes on exit; no-op guard on non-Windows.
- **`resample_to_16k_mono(data: np.ndarray, samplerate: int) → bytes`** goes in `web/audio_writer.py` next to the existing downsampler: average channels → `scipy.signal.resample_poly(x, up, down)` with `up/down = 16000/gcd, rate/gcd` → clip/round to `<i2` bytes. scipy is already a dependency. Keep the existing 48k function untouched (Discord path).
- **`Recording` model changes** (`models.py` + `recording_manager.py` serialization): add `source: str = "discord"` and `devices: dict = field(default_factory=dict)` (chosen device names, display-only). Deserialize with `d.get("source", "discord")` / `d.get("devices", {})` so legacy JSON loads unchanged. `create_recording()` grows optional `source=`/`devices=` params; local sessions pass `voice_channel_id=""`, `guild_id=""`. `reconcile_on_startup()` is already source-agnostic — no change.
- **Mutual exclusion**: one *capture* session at a time across both managers — `start_session` on either manager must 409 if the other has an active recording. Check via the other manager's `active_recording` (both are on `app.state`).
- **Routes** (`web/routes/record.py`): `GET /api/record/devices` → `{"microphones": [{id,name}], "loopbacks": [{id,name}], "available": bool}` — `available: false` (200, not 5xx) when soundcard is not importable, so the Record page can hide the Local section. `POST /api/record/start-local` (JSON: `mic_id`, `system_id`, `campaign_slug?`) → 201 with the same `_recording_to_dict` shape; 409 on active session; 503 when unavailable. `POST /api/record/stop-local` mirroring `/api/record/stop`. Device IDs are passed to soundcard, **never used in file paths** — no path-guard needed on them, but keep them out of error responses per CLAUDE.md.
- **Record page** (`record.html`): "Local" card beside the Discord card — mic `<select>`, system-output `<select>` (populated from `/api/record/devices`), Start/Stop, hidden entirely when `available: false`. Reuse the existing 1 Hz `/record/sse` status stream (it reads `active_recording` generically — verify it points at whichever manager is active). Recording list/detail templates: show "Local" badge vs guild/channel line when `source == "local"`. **Tailwind rebuild + commit `tailwind.min.css`** per CLAUDE.md.
- **Finalise** = BotManager's `_finalise` minus Discord specifics: close all writers, `concat_wav_segments()` on the combined dir → `recordings/<id>/combined.wav` → `combined_path`, status `recording → completed`, always `save_recording()`. The existing transcribe hand-off then works unchanged.
- **`pyproject.toml`**: `[project.optional-dependencies] live = ["SoundCard>=0.4.3"]`.
- **Tests** (`tests/test_local_capture.py` + route additions in `test_record_routes.py`): resampler unit tests (48k stereo, 44.1k non-integer ratio, mono passthrough, empty); manager with scripted sources + instant ticker (per-track WAVs written with correct params, silence substitution when one FIFO starved, combined duration == tick count × 20 ms, stop → completed + `combined_path` set, no-frames session → `combined_path is None`); serialization roundtrip incl. legacy-JSON default `source="discord"`; routes (devices endpoint with fake soundcard in `sys.modules`, unavailable → `available: false`, start-local 201/409/503, cross-manager 409). No real devices/audio anywhere.
- **Docs in the same commit**: architecture.md module map + Recording-layer section + config keys if any; docs/web-ui.md (Local recording section), docs/setup.md (`[live]` extra + per-OS device notes incl. BlackHole), docs/configuration.md if any env var appears; Known Constraints row (native-install only; Docker excluded).
